...
#convert don't know/refused in hscntcr1,2 and not applicable in hscntcr to NA
for (i in 2009:2011){
  temp <- eval(parse(text=paste0('p',i)))
  hs <- names(temp)[grepl('hscntcr',names(temp))==T]
  for(name in hs){
    temp[temp[,name] %in% c("not applicable","don't know","refused"),name] <- NA
    na <- round(sum(is.na(temp[,name]))/nrow(temp)*100,2)
    h1 <- sprintf("Year %d, variable %s, proportion of don't know/refused/not applicable and converted to NA:%.2f%%",i,name,na)
    print(h1)
  }
  assign(paste0('p',i), temp)
}
...
#convert don't know/refused in any observation to NA (daisy can handle NAs)

for (i in 2009:2011){
  temp <- data.frame(eval(parse(text=paste0('q',i))))
  var <- names(temp)
  for (name in var){
    temp[temp[,name] %in% c("don't know","refused"),name] <- NA
    temp[,name] <- as.factor(temp[,name])
    na <- round(sum(is.na(temp[,name]))/nrow(temp)*100,2)
    h1 <- sprintf("Year %d, variable %s, proportion of don't know/refused and converted to NA:%.2f%%",i,name,na)
    print(h1)
  }
  h2 <- sprintf("Year %d, final no. of observations: %d",i,nrow(temp))
  print(h2)
